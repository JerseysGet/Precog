...
for file_path in sorted(os.listdir("./dataset/pretime")):
    ...
    (nodect, edgect) = fu.read_nodes_edges(os.path.join(PREFIX, file_path))
    nodes.append(nodect)
    edges.append(edgect)
...
r_squared_text = (
    f"$R^2 = {lin_reg_model.score(log10_nodes.reshape(-1, 1), log10_edges):0.3f}$"
)
...
plt.plot(
    lineX,
    lineY,
    label=f"${pow(10, lin_reg_model.intercept_):.2g} x ^{exponent}$\n{r_squared_text}",
)

print("R^2 = ", lin_reg_model.score(log10_nodes.reshape(-1, 1), log10_edges))

plt.title("Number of edges vs Number of vertices (log)")
...
plt.ylabel("Number of edges")
plt.legend()
plt.savefig("images/edges_vs_vertices_log.png")
plt.figure()

plt.title("Number of edges vs Number of vertices")
plt.plot(nodes, edges, label="edges")
plt.plot(
    list(range(5000, 35000 + 1)),
    [
        pow(10, lin_reg_model.intercept_) * pow(x, lin_reg_model.coef_[0])
        for x in range(5000, 35000 + 1)
    ],
    label=f"${pow(10, lin_reg_model.intercept_):.2g} x ^{exponent}$",
)
plt.xlabel("Number of vertices")
plt.ylabel("Number of edges")
plt.legend()
plt.savefig("images/edges_vs_vertices.png")
plt.show()
